 ...
 $(".close").click(function() {
    ...
    function editavatar() {
        ...
        $('#province').change(function(){
            ...
            if(id != 0){
                $.ajax({
                    url:'https://api.tuwan.com/playteach/?data=getchina&format=jsonp&id='+id,
                    dataType: "jsonp",
                    success: function (obj){
                        var str = '';
                        for(var x in obj){
        ...
        function get_area_grading(gameID, area_id){
            $.ajax({
                url:"https://app.tuwan.com/lolapi/api/getRank.ashx?player="+gameID+"&areaid="+area_id,
                dataType:'jsonp',
                success:function (obj){
                    if(obj.status == 0){
                        if(obj.rank_num != 0){
        ...
        function subform(){
            ...
            if ($('#nickname').val().trim().length==0 || $('#nickname').val().trim().length>20){
                $('#nickname').addClass('add_input');
                alert('请填写昵称');
                flag = false;
            }
            ...
            var pregqq = /^\d{5,10}$/;
            if (!pregqq.test($('#qq').val())){
                $('#qq').addClass('add_input');
                alert('请填写QQ账号');
                flag = false;
            }
            //标签限制
